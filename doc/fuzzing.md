# Fuzzing BitElonCoin Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing BitElonCoin Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):
...
# libFuzzer" on https://github.com/biteloncoin/biteloncoin/blob/master/doc/fuzzing.md#macos-hints-for-libfuzzer
$ make
...
[`process_message`](https://github.com/biteloncoin/biteloncoin/blob/master/src/test/fuzz/process_message.cpp) is a fuzzing harness for the [`ProcessMessage(...)` function (`net_processing`)](https://github.com/biteloncoin/biteloncoin/blob/master/src/net_processing.cpp). The available fuzzing harnesses are found in [`src/test/fuzz/`](https://github.com/biteloncoin/biteloncoin/tree/master/src/test/fuzz).
...
The project's collection of seed corpora is found in the [`biteloncoin-core/qa-assets`](https://github.com/biteloncoin-core/qa-assets) repo.

To fuzz `process_message` using the [`biteloncoin-core/qa-assets`](https://github.com/biteloncoin-core/qa-assets) seed corpus:

```sh
$ git clone https://github.com/biteloncoin-core/qa-assets
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`biteloncoin-core/qa-assets`](https://github.com/biteloncoin-core/qa-assets) repo.

Every single pull request submitted against the BitElonCoin Core repo is automatically tested against all inputs in the [`biteloncoin-core/qa-assets`](https://github.com/biteloncoin-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make BitElonCoin Core more robust.
...
with certain assembly code from BitElonCoin Core's code. See [developer notes on sanitizers](https://github.com/biteloncoin/biteloncoin/blob/master/doc/developer-notes.md#sanitizers)
...
# Fuzzing BitElonCoin Core using american fuzzy lop (`afl-fuzz`)

## Quickstart guide

To quickly get started fuzzing BitElonCoin Core using [`afl-fuzz`](https://github.com/google/afl):
...
# Fuzzing BitElonCoin Core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing BitElonCoin Core using [Honggfuzz](https://github.com/google/honggfuzz):
...
## Fuzzing the BitElonCoin Core P2P layer using Honggfuzz NetDriver

Honggfuzz NetDriver allows for very easy fuzzing of TCP servers such as BitElonCoin
Core without having to write any custom fuzzing harness. The `biteloncoind` server
...
```sh
$ mkdir biteloncoin-honggfuzz-p2p/
$ cd biteloncoin-honggfuzz-p2p/
$ git clone https://github.com/biteloncoin/biteloncoin
$ cd biteloncoin/
$ ./autogen.sh
$ git clone https://github.com/google/honggfuzz
...
$ git apply << "EOF"
diff --git a/src/biteloncoind.cpp b/src/biteloncoind.cpp
index 455a82e39..2faa3f80f 100644
--- a/src/biteloncoind.cpp
+++ b/src/biteloncoind.cpp
...
EOF
$ make -C src/ biteloncoind
$ mkdir -p inputs/
$ honggfuzz/honggfuzz --exit_upon_crash --quiet --timeout 4 -n 1 -Q \
      -E HFND_TCP_PORT=18444 -f inputs/ -- \
          src/biteloncoind -regtest -discover=0 -dns=0 -dnsseed=0 -listenonion=0 \
