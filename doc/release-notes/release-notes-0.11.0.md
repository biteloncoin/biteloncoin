BitElonCoin Core version 0.11.0 is now available from:

  <https://biteloncoin.org/bin/biteloncoin-core-0.11.0/>
...
For example, add the following to `biteloncoin.conf`:
...
related to the blockchain in the biteloncoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows BitElonCoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that BitElonCoin Core will 
...
For an example script see [biteloncoin-submittx](https://github.com/laanwj/biteloncoin-submittx).
...
- #5334 `cf87536` libbiteloncoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new biteloncoin-detached-sigs git repo for OSX signatures
...
- #5753 `0f44672` Add biteloncoin logo to about screen
...
- #5809 `46bfbe7` Add biteloncoin-cli man page
...
- #5849 `41259ca` contrib/init/biteloncoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename BitElonCoin-Qt.app to BitElonCoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for biteloncoin-tx)
