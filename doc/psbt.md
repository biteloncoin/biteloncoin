# PSBT Howto for BitElonCoin Core

Since BitElonCoin Core 0.17, an RPC interface exists for Partially Signed BitElonCoin
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/biteloncoin/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for BitElonCoin transactions that are not fully signed
...
#### Multisig with multiple BitElonCoin Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
BitElonCoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `biteloncoin-cli` is used.
