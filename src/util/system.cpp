...
const char * const BITELONCOIN_CONF_FILENAME = "biteloncoin.conf";
const char * const BITELONCOIN_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/biteloncoin/biteloncoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //biteloncoin-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", BITELONCOIN_SETTINGS_FILENAME);
...
#else
    const char* pszModule = "biteloncoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows: C:\Users\Username\AppData\Roaming\BitElonCoin
    // macOS: ~/Library/Application Support/BitElonCoin
    // Unix-like: ~/.biteloncoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitElonCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/BitElonCoin";
#else
    // Unix-like
    return pathRet / ".biteloncoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", BITELONCOIN_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure BitElonCoin Core copyright is not removed by accident
    if (copyright_devs.find("BitElonCoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The BitElonCoin Core developers";
    }
