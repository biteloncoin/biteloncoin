This directory contains the source code for the BitElonCoin Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
#### biteloncoingui.(h/cpp)

- Represents the main window of the BitElonCoin UI.
...
- ClientModel is used by the main application `biteloncoingui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a biteloncoin:... link from a browser).
...
* `biteloncoinstrings.cpp`: automatically generated
* `biteloncoinunits.(h/cpp)`: BTC / mBTC / etc. handling
* `callback.h`
...
**Note:** Do not change `local/biteloncoin_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `biteloncoin-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `biteloncoin-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `biteloncoin-qt`
9. You're all set! Start developing, building, and debugging the BitElonCoin Core GUI
