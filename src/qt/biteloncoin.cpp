...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. biteloncoin_de.qm (shortcut "de" needs to be defined in biteloncoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. biteloncoin_de_DE.qm (shortcut "de_DE" needs to be defined in biteloncoin.qrc)
...
static const char* qt_argv = "biteloncoin-qt";
...
void BitcoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success)
    {
        ...
        // biteloncoin: URIs or payment requests:
        if (paymentServer) {
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(biteloncoin);
    Q_INIT_RESOURCE(biteloncoin_locale);
    ...
    /// 6. Determine availability of data directory and parse biteloncoin.conf
    ...
    // biteloncoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
