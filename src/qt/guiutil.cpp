...
#include <qt/biteloncoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a BitElonCoin address (e.g. %1)").arg(
...
{
    // return if URI is not valid or is no biteloncoin: URI
    if(!uri.isValid() || uri.scheme() != QString("biteloncoin"))
        return false;
...
QString formatBitcoinURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("biteloncoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;
...
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", BITELONCOIN_CONF_FILENAME));
    ...
    /* Open biteloncoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "BitElonCoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "BitElonCoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("BitElonCoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for BitElonCoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "biteloncoin.desktop";
    return GetAutostartDir() / strprintf("biteloncoin-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a biteloncoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=BitElonCoin\n";
        else
            optionFile << strprintf("Name=BitElonCoin (%s)\n", chain);
